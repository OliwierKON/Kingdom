        ...
        public class Game {
            ...
            private int Turn = 0; //Turn Count for later
              public void Menu() {
                  ...
                  while(prompt) {
                      Meters health = new Meters("Health/Safety", 100, 50); //meters inside prompt in case i want to add a try again screen (this way they reset when prompt = true again)
                      ...
                      try {  int choice = scan.nextInt();
                           if(choice == 1) {
                              System.out.println();     //turns off the menu ,
                                prompt = false;
                               playTurn(health,money,religion,happy); //starts the game turns
                          } else if (choice==2) {
              ...
              private ArrayList<String>allNames=new ArrayList<>(); //Array list with names
            {
                allNames.add("Gertrude");allNames.add("Martha");allNames.add("Martin");allNames.add("Klortler");allNames.add("HeebieJeebies");allNames.add("Geez");allNames.add("Louise");allNames.add("Mark");allNames.add("Jort");allNames.add("Kendolph");allNames.add("Emineminem");allNames.add("Trevor");allNames.add("Derreanrer");
            }
            ...
            private void playTurn(Meters health, Meters money, Meters religion, Meters happy){
